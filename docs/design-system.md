...
| focus(キーボード操作)/タップ(モバイル) | **未定義**(`style.css` に `focus`/`outline` の指定が一切ない) | 全インタラクティブ要素がブラウザ既定の `:focus` アウトライン・`-webkit-tap-highlight-color` に依存しており、パレット外の色(ブラウザ/OS依存、多くは青系)が表示される。既知の課題(7節)・Issue #66 参照。新規実装でこの節を触る場合は、`--color-accent` 等の既存パレットを用いた `:focus-visible` スタイルの追加を検討すること。 |
...
- **フォーカス/タップ時の視覚状態が未定義**(5節参照):`style.css` に `focus`/`outline` の指定がなく、キーボードフォーカス時のアウトラインやモバイルのタップハイライトはブラウザ既定色に委ねられている。この既定色はプロダクトのカラーパレット(1節)に含まれない。PR #58 でハンバーガーメニュー(`.app-nav-toggle`)を追加した際、狭い画面幅でこの既定色が目立つ形で露出したのを機に判明した(Issue #66)。実装のドキュメント化のみ本節で行い、実際の `:focus-visible` スタイル追加は別PRで対応する。
...
5. **状態表現**: 5節のパターン(hover/disabled/エラー/current/done/active)に倣う。色だけに依存する新しい状態を追加する場合、7節を踏まえてテキストやアイコンなど色以外の手がかりも検討する。**新しいインタラクティブ要素(ボタン・リンク等)を追加する際は、キーボードフォーカス時の見た目をブラウザ既定に委ねず、パレット内の色(`--color-accent` 等)で明示的に定義できないか検討する**(5節の focus/タップの既知の課題を参照)。
...
7. **アクセシビリティ最低基準**(7節の課題を踏まえた、新規実装時の最低ライン):
   ...
   - キーボードフォーカス時のインジケータを、パレット外のブラウザ既定色に頼らず明示的に定義する(5節の既知の課題を参照)。
